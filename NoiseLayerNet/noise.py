...
class GaussianBlur2D(nn.Module):
    def __init__(self, kernel_size=3, sigma=0.4, channels=3, height=resize_h, width=resize_w):  # 3*c.clip_len
...
if __name__ == '__main__':
    mode = GaussianNoise().to('cuda:0')
    x = torch.randn(4, 3, 224, 224).to('cuda:0')
    # x = torch.randn(4, 3, 256, 256).to('cuda:0')
    y = mode(x)
    print(y.shape)  # torch.Size([4, 3, 256, 256])

    mode = GaussianBlur2D().to('cuda:0')
    x = torch.randn(4, 3, 224, 224).to('cuda:0')
    # x = torch.randn(4, 3, 256, 256).to('cuda:0')
    y = mode(x)
    print(y.shape)  # torch.Size([4, 3, 256, 256])